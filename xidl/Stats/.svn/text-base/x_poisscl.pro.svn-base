...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
function x_poisscl, x, cl, SIGMA=sigma, NSPL=nspl, SILENT=silent, VERBOSE=verbose
  ...
  if keyword_set( SIGMA ) then begin
     cl = gauss_pdf(sigma)
     if keyword_set(VERBOSE) then print, 'x_poisscl: Using a CL of ', cl
  endif
...
;  norm = 1. - igamma(x+1,x)  ;; Note x! divided out
  ...
  luparr = lup # replicate(1., n+1)
  narr = replicate(1., nspl) # dindgen(n+1)

  summarr = luparr^narr * exp(-1.*luparr) / gamma(narr+1)
  ...
  llowarr = llow # replicate(1., n)
  narr = replicate(1., nspl) # dindgen(n)

  summarr = llowarr^narr * exp(-1.*llowarr) / gamma(narr+1)
