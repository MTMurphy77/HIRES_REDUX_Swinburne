...
pro lris_sensstd, std_fil, CLOBBER=clobber, CONDITIONS=conditions, $
                  ...
                  WVTIME=wvtime, ALL_GRATING=all_grating, STD_OBJ=std_obj, $
                  CALIBFIL=calibfil
...
;  stop
  ...
  if x_chkfil(outfil+'*') and not keyword_set(CLOBBER) then begin
      ...
      if not keyword_set(NO_UPD_WEB) then begin
          files = findfile(getenv('LRIS_THRU')+'/'+lbl+'/sens_LRIS*')
          mkhtml_specthru, files, TITLE='LRIS '+lbl+' Throughput Measurements', $
                           WVTIME=WVTIME, ALL_GRATING=all_grating, $
                           OUTPTH = getenv('LRIS_THRU')+'/'+lbl+'/', $
                           OUTFIL = getenv('LRIS_THRU')+'/'+lbl+ $
                           '/thru_summ.html'
      endif
      return
  endif 
