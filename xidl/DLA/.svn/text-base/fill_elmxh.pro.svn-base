...
; Create the abundance array
  ...
  for j=0,ndla-1 do begin
      ...
      ;; Abund fil
      a = findfile(strtrim(stddla[j].abndfil,2),count=na)
      if na EQ 0 then begin
          print, 'fill_elmxh: No Abund file -- ', stddla[j].abndfil
          continue
      endif

      openr, 2, strtrim(stddla[j].abndfil,2)
      readf, 2, dumc
      readf, 2, dumi
      stddla[j].ndfil = dumi
      ...
      if numlines(root+XHfil) EQ 0 then begin
          close, 2
          continue
      endif
