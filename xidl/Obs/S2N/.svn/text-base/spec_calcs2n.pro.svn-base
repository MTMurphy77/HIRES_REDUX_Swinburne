...
function generate_template, str_obs, wave
  ...
  temp_mstar = single_spec2mag(temp.wavelength,temp.flux*1e17,filtfile=str_obs.filter)
  ...
  if str_obs.mtype EQ 1 then begin
     ...
     vega_mstar = single_spec2mag(vega.wavelength,vega.flux*1e17,filtfile=str_obs.filter)
