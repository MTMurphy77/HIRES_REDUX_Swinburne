...
function sdss_ewciv, wave, flux, sig, conti, dblt_name, zabs, istrct=istrct, $
                     ...
                     keepwvlim=keepwvlim, final=final, generic=generic

  if  N_params() LT 6  then begin 
     print,'Syntax - ' + $
           'sdss_ewciv(wave, flux, sig, conti, dblt_name, zabs, '
     print,'                  [zlim=, snr_conv=, istrct=, /plot, /debug, /generic])'
     return,-1
  endif 
  ...
  wrtag = where(tags eq 'WREST')
  if wrtag[0] eq -1 then begin
  ...
  if keyword_set(final) and not keyword_set(generic) then begin
     ...
     ncolmtag = where(tags eq 'NCOLM_FINAL')
     signcolmtag = where(tags eq 'SIGNCOLM_FINAL')
  endif else begin
     ...
     ncolmtag = where(tags eq 'NCOLM_ORIG')
     signcolmtag = where(tags eq 'SIGNCOLM_ORIG')
  endelse 
  ...
  for ii=0L,nlin-1 do begin
     ...
     if keyword_set(keepwvlim) then begin
        ...
        if keyword_set(snr_conv) then begin
           ...
           if ngd gt 0 then ilhs = gd[ngd-1]
           ...
           if ngd gt 0 then irhs = irhs + gd[0]
        ...
        ;; doesn't beecome ridiculous
        bd = where(flux[0:icen]+sig[0:icen] gt conti[0:icen] and conti[0:icen] ne 0 and $
                   finite(conti[0:icen]),nbd)
        if nbd ne 0 then ilhs_test = bd[nbd-1] $
        else ilhs_test = ilhs
        bd = where(flux[icen:*]+sig[icen:*] gt conti[icen:*] and conti[icen:*] ne 0 and $
                   finite(conti[icen:*]),nbd)
        if nbd ne 0 then irhs_test = icen + bd[0] $
        else irhs_test = irhs
     ...
     gd = where(sig[ilhs:irhs] ne 0. and flux[ilhs:irhs] lt $
                conti[ilhs:irhs])
     if gd[0] ne -1 then begin
        ...
        gd = where(sig[ilhs:irhs] ne 0.)
        if gd[0] ne -1 then $
           wgt[gd] = 1. $       ; only good portion
        else wgt[*] = 1.        ; all portions
        if keyword_set(debug) then $
           print,'sdss_ewciv debug: uniform weighting'
     endelse 
     ...
     ;; EW measurement
     qstrct.(ewtag)[ii] = total( (1.-flux[ilhs:irhs]/conti[ilhs:irhs]) $
                                 *dwv[ilhs:irhs]) 
     qstrct.(sigewtag)[ii] = sqrt(total( (sig[ilhs:irhs]/conti[ilhs:irhs] $
                                          *dwv[ilhs:irhs])^2)) 
     ...
     if ncolmtag[0] ne -1 then begin
        ;; Column density by AODM
        x_aodm,wave[ilhs:irhs],flux[ilhs:irhs]/conti[ilhs:irhs],$
               sig[ilhs:irhs]/conti[ilhs:irhs],qstrct.(wrtag)[ii,cindx],$
