...
function sdss_fndlin_srch, contistrct_fil, cflg, wave=wave, flux=flux, $
  ...
  mn = min(abs(wave-5579.), i55)
  mn = min(abs(wave-6302.), i63) ; s to match sdss_fndciv...
  bdbracket = [i55-lindgen(6),i55+lindgen(5)+1, i63-lindgen(3),i63+lindgen(2)]
...
function sdss_fndlin_fitspline, wave, flux, sigma, zqso, premask=premask, chi_sqr=chi_sqr, $
  ...
  if n_params() ne 4 then begin
     ...
     print,'                  pca_fil=, pca_head=, qualflg=, '
  ...
  if ipend GT 200 then begin
     ...
     if keyword_set(debug) then $
        print,'sdss_fndlin_fitspline() debug: this is a low-z QSO, zqso = '+$
              string(zqso,format='(f8.5)') 
     norm = flux[*]
     nrm_ivar = ivar[*]
     fconti = replicate(1.,  (size(flux,/dim))[0])
...
pro sdss_fndlin, sdss_list, sdsssum, tcpu=tcpu, clobber=clobber, $
  ...
  for qq=istrt,nfil-1 do begin
     ...
     for cc=0,2 do begin
        ...
        ;; propogate errors
        ivconti = 1./cstrct.conti[0:cstrct.npix-1,cc]
        fconti = cstrct.conti[0:cstrct.npix-1,cc]
        fx = flux*ivconti
        ;; sig = sqrt(sigma^2*cstrct.conti[0:cstrct.npix-1,cc]^2 + $
        ;;            cstrct.sigconti[0:cstrct.npix-1,cc]^2*flux^2)*ivconti^2
;        if bdpix[0] ne -1 then sig[bdpix] = 9.e9 ; keep ivar real below
