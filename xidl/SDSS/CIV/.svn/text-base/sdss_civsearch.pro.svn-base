...
function sdss_civsearch_broad, contistrct_fil, dblt_name, count=count, $
  ...
  print,'sdss_civsearch_broad(): z limits ',string(zmin,zmax,format='(f8.5,"--",f8.5)')
...
;  sig = sdss_calcnormerr(flux, sigma, cstrct, /unnorm, cflg=cstrct.cflg)
  ...
  for ii=0L,count-1 do begin
     ...
     civstr[ii] = sdss_ewciv(wave, flux, sigma, conti, dblt, $
                             civstr[ii].zabs_orig[0], istrct=civstr[ii], $
...
function sdss_civsearch_srch, wave, flux, sigma, dblt_name, abslin_fil, $
  ...
  ;; sdss_fndciv: _EXTRA include dvtol=, dvgal=, dvqso=, dz=
  ...
  conti = cstrct.conti[0:cstrct.npix-1,cindx] 
  ;; sigma = sqrt(sigma^2*cstrct.conti[0:cstrct.npix-1,cindx]^2 + $
  ;;              cstrct.sigconti[0:cstrct.npix-1,cindx]^2*flux^2)/$
  ;;         cstrct.conti[0:cstrct.npix-1,cindx] ; "unnorm" total error
  sigma = sdss_calcnormerr(flux,sigma,cstrct,/unnorm,cflg=cstrct.cflg)
  if keyword_set(debug) then begin
  ...
  if not keyword_set(excl_broad) then begin
     ...
     civstr = sdss_civsearch_broad(cstrct, dblt, count=nbroad, $
                                   wave=wave, flux=flux, sigma=sigma, $
  ...
  qstrct = sdss_fndciv(cstrct.centroid[0:cstrct.ncent[cindx]-1,cindx], $
                       ...
                       wave=wave, flux=flux/conti, sigma=sigma/conti, $
  ...
  if qstrct.nDLA2 gt 0 then begin
     ...
     for kk=0L,qstrct.ndla2-1 do $
        ...
        civstr[kk] = sdss_ewciv(wave, flux, sigma, conti, dblt, $
                                qstrct.dla_zabs2[kk], $
...
pro sdss_civsearch, filename, sdsssum, cand_fil, zmin=zmin, $
  ...
  gd = where(sdsstab.z GE zmin AND sdsstab.rtmag LT Rmax AND $
...
;        if keyword_set(debug) then $
;           print,'sdss_civsearch debug: broad CIV count = '+strtrim(n_elements(allbroadciv),2)
     endif 
     if flg_broadciv eq 1 then print,n_elements(allbroadciv)
