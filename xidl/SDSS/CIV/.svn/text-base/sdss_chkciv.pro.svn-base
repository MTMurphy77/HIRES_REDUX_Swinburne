...
pro sdss_chkciv_icmmn, state, fail=fail
  ...
  qalstr.rating[[0,9]] = -1     ; rating and blend flag storage locales
...
pro sdss_chkciv_blend, state
  ...
  if civstr[state.curqal].rating[9] ne 1 then nwflg = 1 $
  else nwflg = 0
        ...
        if cstrct.ncent[cindx] gt 0. then begin
           ...
           tmp = cstrct.centroid[$
                 sort(cstrct.centroid[0:cstrct.ncent[cindx]-1,$
                                      cindx]),cindx]
           wv_lin[2*lindgen(cstrct.ncent[cindx])] = tmp
           if cstrct.ncent[cindx] gt 1 then begin
              ;; Prevent x_spepclot's sorting from screwing this up
              ;; by toggling a little
              wv_lin[2*lindgen(cstrct.ncent[cindx])+1] = tmp       
              
              fx_lin[4*lindgen(cstrct.ncent[cindx]/2.)] = 500. ; just something large
              wv_lin[4*lindgen(cstrct.ncent[cindx]/2.)] = $
                 wv_lin[4*lindgen(cstrct.ncent[cindx]/2.)] - 5.e-4
              
              fx_lin[4*lindgen(cstrct.ncent[cindx]/2.)+1] = -500. ; just something small       
              wv_lin[4*lindgen(cstrct.ncent[cindx]/2.)+1] = $
                 wv_lin[4*lindgen(cstrct.ncent[cindx]/2.)+1] - 4.e-4
              
              fx_lin[4*lindgen(cstrct.ncent[cindx]/2.)+2] = -500. 
              wv_lin[4*lindgen(cstrct.ncent[cindx]/2.)+2] = $
                 wv_lin[4*lindgen(cstrct.ncent[cindx]/2.)+2] - 3.e-4
              
              fx_lin[4*lindgen(cstrct.ncent[cindx]/2.)+3] = 500.       
              wv_lin[4*lindgen(cstrct.ncent[cindx]/2.)+3] = $
                 wv_lin[4*lindgen(cstrct.ncent[cindx]/2.)+3] - 2.e-4
           ...
           wv_lin = [0.,0.]
           fx_lin = [0.,0.]
           print,'sdss_chkciv: no centroids detected for current LOS.'
        endelse 
     endif else begin
...
pro sdss_chkciv_llist, state
  ...
  case state.dblt.ion of 
     ...
     else: stop,'sdss_chkciv_llist: state.dblt.ion DNE ',state.dblt.ion
  endcase 
...
pro sdss_chkciv_setciv, state, DEFINITE=definite, GOOD=good, MAYBE=maybe, BAD=bad
  ...
  endelse                       ; had to do something
   ...
   civstr[state.curciv].rating[0] = flg_civ
...
pro sdss_chkciv, qalfil, civfil, dblt_name=dblt_name, hidealt=hidealt, $
  ...
  if  N_params() LT 2  then begin 
     print,'Syntax - sdss_chkciv, qalfil, civfil, [dblt_name=, /debug, '
     print,'                  altdblt_name=, /sav_ech, xsize=, ysize=]'
     return
  endif 
...
;  state.help[27] = 'R or C -- Set right bound of 1550 EW region.'
  ...
  tmp = where(civstr.rating[9] eq 1,nblnd)
