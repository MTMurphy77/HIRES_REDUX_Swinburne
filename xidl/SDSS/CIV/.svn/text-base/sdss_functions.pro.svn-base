...
pro sdss_mtchcivstrct, strct1_fil, strct2_fil, mtch_fil, diff_fil, $
  ...
  ;; Defaults
  if not keyword_set(dvtol) then dvtol = 0.d ; km/s fuzziness
  cinv = 1./3.e5                             ; km^-1 s
  ...
  if keyword_set(mask_fil) then begin
     ...
     for ll=0L,nloop-1 do begin
        if ngdsnr lt nmask_spec then begin
           ...
           if ntmch eq 1 then mtch = mtch[0]
        endif else begin
...
function sdss_calcdzlos, z_qso, dblt_name, zlim, z_global, spec_fil, rz_los=rz_los,$
  ...
  if keyword_set(spec_fil) or keyword_set(wave) then begin
     ...
     badpix = where(error le 0. and redshift ge min(zlim_los,max=mx) and $
                    redshift le mx,nbadpix) ; hope SDSS and sdss_calcnormerr() did this right
     if nbadpix ne 0 then begin
        ...
        tmp_zlim_los = dblarr(2,nbin+ngaps,/nozero)
        tmp_zlim_los[*,0:nbin-1] = zlim_los
        tmp_zlim_los[0,nbin:*] = redshift[istart]
        tmp_zlim_los[1,nbin:*] = redshift[istop]
        ...
        for gg=1,nbin+ngaps-1 do begin
           ;; Have to handle
           if tmp_zlim_los[0,gg] lt new_zlim_los[1,count] then $ ; next lower < prev upper
              new_zlim_los[1,count] = tmp_zlim_los[1,gg] $       ; so use next upper
           else begin
              ;; increment
              count++
              new_zlim_los[*,count] = tmp_zlim_los[*,gg]
           endelse
        endfor                  ; loop gg=1,nbin+ngaps-1

        if keyword_set(debug) and count ne nbin+ngaps-1 then begin
           print,'sdss_calcdzlos() debug: Old and new LOS z limits, excluding gaps'
        ...
        if count eq 0 then zlim_los = reform(zlim_los) $ ; simple array
        else done = 0                                    ; must transpose
     endif                      ; nbadpix != 0
     ...
     for zz=0,nzlim-1 do begin
        ...
        if zz eq 0 then begin
           ...
           if iz_los[1] eq iz_tmp[0] then begin
              ;; The sky line gap falls in same bin so summ
  ...
  for zz=0,cmpltstr.nzbin-1 do begin
     ...
     if keyword_set(civobs_corr) then begin
        ...
        iciv = where(civstr.(zabstag)[0] ge z_global[zz] and $
                     civstr.(zabstag)[0] lt z_global[zz]+dz_global[zz],niciv)
...
function sdss_calcdndxfit, ewlim, option=option, fitstrct_fil=fitstrct_fil, $
  ...
  if n_params() ne 1 then begin
   ...
   case strlowcase(option) of
      ...
      'sch': begin
         ;; incomplete gamma function igamma(1+alpha, data/datanorm)
         ;; dN/dX = -coeff*datanorm*(igamma(1+alpha,ewrng[*,1]/datanorm)
         ;; - igamma(1+alpha,ewrng[*,0]/datanorm)
         rslt = -1.*datanorm*($
                igamma(1+alpha, ewrng[*,1]/datanorm,/double) - $
                igamma(1+alpha, ewrng[*,0]/datanorm,/double))
         ndim = 3
      end
   endcase
...
;; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
function sdss_getcivstrct, strct_fil, rating=rating, zlim=zlim, ewlim=ewlim, $
  ...
  if keyword_set(help) then begin
     print,'Syntax - sdss_getcivstrct([strct_fil, rating=, zlim=, ewlim=, '
     print,'                           nlim=, dvgal=, dvqso=, /noBAL,/unblend, /final, /dropbox,'
...
;; ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
function sdss_calcfxwchisq, civstrct_fil, cmplt_fil, fxw=fxw, zlim=zlim, $
                            _extra=extra
...
function sdss_applyuserbias, cmpltstr_fil, biasuser_fil, _extra=extra
  ...
  for zz=0,cmpltstr.nzbin-1 do begin
     ...
     ;; figure out which user bias bin to use
     user_indx = where(zabs[0] ge userbiasstr.zlim[*,0] and $
                       zabs[0] lt userbiasstr.zlim[*,1],nmtch)
     if nmtch gt 1 then begin
        tmp = 0.5*total(userbiasstr.zlim,2)       ; average
        mn = min(tmp[user_indx] - zabs[0],imn,/abs) ; closest
        user_indx = user_indx[imn]
     endif else user_indx = user_indx[0]
     print,'zabs, user_indx',zabs[0],user_indx
     coeff = reform(userbiasstr.coeff_tp[user_indx,*,*]) ; [2,3]
...
function sdss_adjafpfxwfit, fitstrct, errstrct, newerrstrct=newerrstrct, $
  ...
  afpdndx = sdss_getafpdndx(fitstrct.zrng[[0,2]], fitstrct.intlim[[0,2]], _extra=extra)
...
function sdss_cpZhuMenard2strct,zm12strct_fil,sdsstab,bal_fil=bal_fil,nvdisp=nvdisp,$
                                _extra=extra
  ;; Copy Zhu & Menard (2012, arXiv) MgII & FeII structure to
  ;; sdsscivstrct format 
  ;; http://www.pha.jhu.edu/~gz323/Site/Download_Absorber_Catalog.html

  if n_params() ne 1 then begin
     print,'Syntax - sdss_cpZhuMenard2strct(zm12strct_fil,[sdsstab,bal_fil=,nvdisp=,_extra=])'
     return,-1
  endif
  
  if not keyword_set(nvdisp) then nvdisp = 1. ; Gaussian width

  if size(zm12strct_fil,/type) eq 7 then $
     zm12strct = xmrdfits(zm12strct_fil,1,/silent) $
  else zm12strct = zm12strct_fil
  nzm12 = (size(zm12strct,/dim))[0] > 1 
  ...
  mjd = strtrim(zm12strct.mjd,2) ; may not be spectro MJD

  plate = strtrim(zm12strct.plate,2)
  ...
  fiber = strtrim(zm12strct.fiber,2)
  ...
  zm12qso_name = mjd+'-'+plate+'-'+fiber
  zm12conti_fil = 'abslin/1d_26/'+plate+'/1d/spSpec-'+zm12qso_name+'-abslin.fit' ; may not exist

  ;; Sort 
  srt = sort(zm12qso_name)
  zm12strct = zm12strct[srt]
  zm12qso_name = zm12qso_name[srt]
  zm12unq = uniq(zm12qso_name)
  nzm12unq = (size(zm12unq,/dim))[0] > 1
  nabs_per_zm12 = [zm12unq[0]+1,(shift(zm12unq,-1)-zm12unq)[0:nzm12unq-2]]
  zm12tags = tag_names(zm12strct)
  ...
  mgiistrct = replicate(newstrct,nzm12)
  mgiistrct.rating[0] = sdss_getrating(/def)
  dblt = dblt_retrieve('MgII')
  mgiistrct.wrest[0] = dblt.wvI
  iewmgii_I = (where(zm12tags eq 'REW_MGII_2796'))[0]
  ivdmgii_I = (where(zm12tags eq 'VDISP_MGII_2796'))[0]
  mgiistrct.wrest[1] = dblt.wvII
  iewmgii_II = (where(zm12tags eq 'REW_MGII_2803'))[0]
  ivdmgii_II = (where(zm12tags eq 'VDISP_MGII_2803'))[0]
  dblt = dblt_retrieve('FeII')  ; 2600, 2586
  mgiistrct.wrest[2] = dblt.wvI
  iewfeii_I = (where(zm12tags eq 'REW_FEII_2600'))[0]
  ivdfeii_I = (where(zm12tags eq 'VDISP_FEII_2600'))[0]
  mgiistrct.wrest[3] = dblt.wvII
  iewfeii_II = (where(zm12tags eq 'REW_FEII_2586'))[0]
  ivdfeii_II = (where(zm12tags eq 'VDISP_FEII_2586'))[0]
  mgiistrct.wrest[4] = 2382.765
  iewfeii_III = (where(zm12tags eq 'REW_FEII_2383'))[0]
  ivdfeii_III = (where(zm12tags eq 'VDISP_FEII_2383'))[0]
  mgiistrct.wrest[5] = 2374.4612
  iewfeii_IV = (where(zm12tags eq 'REW_FEII_2374'))[0]
  ivdfeii_IV = (where(zm12tags eq 'VDISP_FEII_2374'))[0]
  mgiistrct.wrest[6] = 2344.214 
  iewfeii_V = (where(zm12tags eq 'REW_FEII_2344'))[0]
  ivdfeii_V = (where(zm12tags eq 'VDISP_FEII_2344'))[0]
  mgiistrct.abslin_fil = zm12conti_fil

  ;; Big loop
  for qq=0L,nzm12unq-1 do begin
     if qq gt 0 then zm12rng = zm12unq[qq-1] + lindgen(nabs_per_zm12[qq]) + 1 $
     else zm12rng = lindgen(nabs_per_zm12[qq])

     ;; Try on name only
     mtch = where(zm12qso_name[zm12rng[0]] eq qso_name,nmtch)

     case nmtch of 
        0: begin
           ;; Find by distance
           gcirc,1,zm12strct[zm12rng[0]].ra*24/360.,zm12strct[zm12rng[0]].dec,$
           ...
           if mindist gt 10. then begin
              ...
              print,'sdss_cpZhuMenard2strct(): Over-riding qso_name match for closest ',$
                    zm12qso_name[zm12rng[0]],' '+qso_name[mtch],mindist
           endif
        end
     endcase
     ...
     if mindist gt 10. then begin
        print,'sdss_cpZhuMenard2strct(): WARNING! no quasar match within 10 arcsec ',$
             zm12qso_name[zm12rng[0]],mindist
        ;; Load up information from Zhu & Menard (2012) structure
        mgiistrct[zm12rng].qso_name = zm12qso_name[zm12rng[0]]
        mgiistrct[zm12rng].mjd = zm12strct[zm12rng[0]].mjd
        mgiistrct[zm12rng].plate = zm12strct[zm12rng[0]].plate
        mgiistrct[zm12rng].fiber = zm12strct[zm12rng[0]].fiber
        mgiistrct[zm12rng].ra = zm12strct[zm12rng[0]].ra
        mgiistrct[zm12rng].dec = zm12strct[zm12rng[0]].dec
        mgiistrct[zm12rng].rmag = -999 ; let this be the warning flag
        ;; skipping SNR (below)
        mgiistrct[zm12rng].sdss_obs[0] = 'spectro/1d_26/'+plate[zm12rng]+'/1d/spSpec-'+$
                                     zm12qso_name[zm12rng]+'.fit'
        mgiistrct[zm12rng].z_qso = zm12strct[zm12rng[0]].zqso
        mgiistrct[zm12rng].z_qso_hw10 = zm12strct[zm12rng[0]].zqso
        ;; skipping BALFLG
     endif else begin
        ;; Load up the information from Schneider et al. (2010) structure
        mgiistrct[zm12rng].qso_name = qso_name[mtch]
        mgiistrct[zm12rng].mjd = sdsssum[mtch].smjd
        mgiistrct[zm12rng].plate = sdsssum[mtch].plate
        mgiistrct[zm12rng].fiber = sdsssum[mtch].fiber
        mgiistrct[zm12rng].ra = sdsssum[mtch].ra
        mgiistrct[zm12rng].dec = sdsssum[mtch].dec
        mgiistrct[zm12rng].rmag = sdsssum[mtch].rtmag
        ;; skipping SNR (below)
        mgiistrct[zm12rng].sdss_obs[0] = spec_fil[mtch]
        mgiistrct[zm12rng].z_qso = sdsssum[mtch].z
        mgiistrct[zm12rng].z_qso_hw10 = zm12strct[zm12rng[0]].zqso
        ;; BALFLG
        test = where(balqso_name eq qso_name[mtch[0]])
        if test[0] ne -1 then mgiistrct[zm12rng].balflg = 12 ; 8 and 4
     endelse
     mgiistrct[zm12rng].abslin_fil = 'abslin'+strmid(spec_fil[mtch],7,36)+'-abslin.fit'

     for ii=0,nabs_per_zm12[qq]-1 do begin
        mgiistrct[zm12rng[ii]].zabs_orig[0:6] = zm12strct[zm12rng[ii]].zabs
        mgiistrct[zm12rng[ii]].sigzabs_orig[0:6] = zm12strct[zm12rng[ii]].err_zabs

        mgiistrct[zm12rng[ii]].snr[0] = zm12strct[zm12rng[ii]].med_sdeviation_red
        mgiistrct[zm12rng[ii]].snr[1] = zm12strct[zm12rng[ii]].med_sdeviation_blue
        mgiistrct[zm12rng[ii]].snr[2] = zm12strct[zm12rng[ii]].spec_snr_median
        ...
        for jj=0,6 do begin
           ...
           mgiistrct[zm12rng[ii]].ew_orig[jj] = zm12strct[zm12rng[ii]].(iew)
           mgiistrct[zm12rng[ii]].sigew_orig[jj] = zm12strct[zm12rng[ii]].(isigew)

           mgiistrct[zm12rng[ii]].gwidth[jj] = zm12strct[zm12rng[ii]].(ivd)
        endfor                  ; loop jj=0,6

     endfor                     ; loop ii=nabs_per_zm12
     
  endfor                        ; loop qq=nzm12unq
...
pro sdss_functions, compile=compile
  ...
  print,'SDSS_CPZHUMENARD2STRCT() -- copy Zhu & Menard (2012) structure to sdsscivstrct.'
