...
pro sdss_completeness_czw, list_fil, snrstrct_fil, cmplt_fil, $
                           ...
                           biasuser_fil=biasuser_fil, _extra=extra
  ...
  if n_params() ne 3 then begin
     ...
     print,'                          user=, /clobber, /debug, /silent, biasuser_fil=, _extra=]'
     return
  end
  sdssdir = sdss_getsdssdir()
...
;     else $
     ...
     if ntest ne 0 then begin
        ...
        for ss=0,mcstr[0].nsys-1 do begin
           ...
           gd = where(mcstr.(ztag)[ss,0] ge zlim[0] and $
                      mcstr.(ztag)[ss,0] lt zlim[1] and $
                      mcstr.(vartag)[ss,0] ge ewlim[0] and $
                      mcstr.(vartag)[ss,0] lt ewlim[1],ngd,$
                      complement=bd,ncomplement=nbd)

           if ngd eq 0 then begin
              print,'sdss_completeness_czw: no system ss='+strtrim(ss,2)+$
                    ' MC profiles in z and EW bounds ',$
                    mcstr[0].qso_name,string(snrstrct[ff].z_qso,format='(f8.5)'),$
           ...
           ;; "Hit" in grid only if mcstr.flg_rec[ss] = 1
...
;              (los_arr.zlim[1] gt zlim[0] and los_arr.zlim[1] le zlim[1]))
  ...
  if keyword_set(civobs_corr) then begin
     ...
     cmpltstr = sdss_applycivobscorr(cmpltstr, civobs_corr, $
...
pro sdss_completeness_cum, n_per_bin, froot, civ_fil, round_scale=round_scale, $
  ...
  sdssdir = sdss_getsdssdir()
  zfudge = 0.001                ; rounding errors in single bins
  ...
  if not keyword_set(nociv) then begin
     ;; Read in data
     ;; _extra includes rating=, zlim=, ewlim=, nlim=, dvgal=, dvqso=,
  ...
  for ii=0,nzbin-1 do begin
     ...
     ;;                  /clobber, /silent, civobs_corr=, [_extra=]
...
pro sdss_completeness_userbias, user_fil, userrate_fil, out_fil, $
  ...
  if n_params() ne 3 then begin
     ...
     print,'                                /debug, /silent,/clobber,'
     print,'                                cmplt_fil=, dvqso=, _extra=]'
     return
  endif 
     ...
     for jj=0,mcstr[ii].nsys-1 do begin
        ...
        if mcstr[ii].flg_rec[jj] eq 1 then begin
           ...
           if mtch[0] ne -1 then begin
              ...
              stop,'sdss_completeness_userbias stop: no match for recovered mcstr ',ii,jj
        endif else begin
  ...
  ;; ;;;;;;;
  ;; Apply dvqso cut
  mcsub = where(mcdvqso lt dvqso,nmcciv,ncomplement=nbd)
  ...
  print,'Total input profiles < dvqso: ',string(nmcciv,format='(i4)')
  print,'Total number automatically recovered (not): ',$
        string(nrec,nrej,format='(i4,1x,"(",i4,")")')
  ...
  print,'Total '+dblt.ion+' candidates: ',string(ncivrate,format='(i4)')
  print,'Number of recovered '+dblt.ion+' matched up (not): ',$
        string(ntrurec_indx,nfpr_indx,format='(i4,1x,"(",i4,")")')
;  sdss_chkciv,civrate[fpr_indx],strlowcase(dblt.ion)+'_fpr_indx.fit',dblt_name=dblt.ion
  ...
  if ntrurec_indx ne 0 then begin
     ...
     print,'    True positives (false negatives): ',$
           string(ntp_indx,nfn_indx,format='(i4,1x,"(",i4,")")')
  endif
  if nfpr_indx ne 0 then begin
     ...
     print,'    True negatives (accepted false positives): ',$
           string(ntn_indx,nafp_indx,format='(i4,1x,"(",i4,")")')
  endif
  ...
  print,'Total rejected '+dblt.ion+' profiles: ',$
        string(ncivrej,format='(i4)')
  print,'Number of rejected '+dblt.ion+' matched up (not): ',$
        string(nmore_input_indx,nunknown_indx,format='(i4,1x,"(",i4,")")')
  ...
  if keyword_set(cmplt_fil) then begin
     ...
     sdss_completeness_cum,zbin,froot,dblt_name=dblt,silent=silent,$
                           ...
                           out_fil=cmpltcum_fil,dvqso=dvqso,_extra=extra
