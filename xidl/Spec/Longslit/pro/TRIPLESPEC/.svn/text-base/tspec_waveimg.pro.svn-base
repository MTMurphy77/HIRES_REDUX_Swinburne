...
;    fsig_order1 = [5.0, 5.0, 5.0, 5.0, 4.0, 4.0] ; order 6 is more liberal
    ...
    spec_set = bspline_iterfit(findgen(ny), arc1d[*,iorder] $
                               , invvar = (arc1d[*,iorder] GE 0.0 AND arc1d[*,iorder] LE 1d6) $
                               , bkspace = bkspace $
                               , yfit = autofit $
...
;                      , toler = toler, CHK = CHK, ONLY_SLITS = [1, 2, 3, 4, 5])
  ...
  uttime = sxpar(hdr, 'TIME')
  mjd = x_setjdate(strmid(utshut,0,10), uttime)
  ;jd = 2400000.5D + sxpar(hdr, 'MJD_OBS') 

  helio = (-1.)*x_keckhelio(ra, dec, equinox, jd = mjd, OBS = 'Palomar')
