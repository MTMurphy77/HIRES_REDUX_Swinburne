...
PRO tspec_reduxplan, plan_file, telluric=telluric, CLOBBER=clobber
  ...
  for qq=0L,nuni-1 do begin
    ...
    for jj=0L,mxi1 do begin
       ...
       if mxi2 NE 3 then begin
          print, 'tspec_reduxplan: Was expecting 3 files but found only', mxi2
          ;stop ;; Should probably almost always be 3
       endif
       for ii=0L,mxi2,2 do begin
          ...
          if keyword_set(TELLURIC) then begin
             ...
             ;; Reduce
             tspec_redux_pair, obj_fil, telluric=rdx_file
          endif else begin
             ;; First one
