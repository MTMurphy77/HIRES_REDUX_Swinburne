FUNCTION SOFI_SKYSUB, afiles, bfiles, ivar = ivar, sky_resids = sky_resids, HDR = HDR $
                      , obj_pos = obj_pos, obj_neg = obj_neg, WAVEIMG = WAVEIMG $
                      , TELLURIC = TELLURIC1, CHK = CHK, WVCHK = WVCHK $
                      , VERBOSE = VERBOSE, targdir = targdir $
                      , AVG_SKY = AVG_SKY, SKY_MODEL = SKY_MODEL, NOMASK = NOMASK1 $
                      , SLITMASK = SLITMASK, tset_slits = tset_slits, wavefile = wavefile $
...
pixset = long_wavepix(avg_sky, tset_slits, FWHM = FWHM, pkwdth = pkwdth, toler = toler $
                      , chk = chk_trc)
...
obj_neg = long_objfind(-a_min_b_fit, tset_slits = tset_slits $
                       , FWHM = fwhm, OBJMASK = OBJMASK_NEG $
                       , OBJTHRESH = KEYWORD_SET(TELLURIC)*0.1 $
                       , NPERSLIT = KEYWORD_SET(TELLURIC), peakthresh = peakthresh)
IF KEYWORD_SET(obj_neg) THEN objtot = [obj_pos, obj_neg] $
ELSE objtot = obj_pos
OBJMASK = (OBJMASK_POS EQ 1) OR (OBJMASK_NEG EQ 1)
