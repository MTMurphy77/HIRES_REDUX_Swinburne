...
function long_2dlocalskysub, sciimg, sciivar, skyimage, piximg, waveimg  $
    ...
    CASE TARG OF
       '0239-0106': BEGIN
          ;;z_fg =  2.2927107     ; f/g quasar redshift
          z_fg = 2.3025                  ; absorber redshift
          z_bg =  3.14          ;;b/g quasar redshift
          dv = 70000.0d*1.0d5
    ...
    lya_ap = 5.0d             ;; lyman alpha masking aperture in arcseconds
    pix_ap = 5.0d/plate_scale ;; lyman alpha aperture in pixels
    ...
    dv_lya = 2000.0d*1.0d5
    lya_min_fg = (1.0d + z_fg - dv_lya/c.c)*1215.67d
    lya_max_fg = (1.0d + z_fg + dv_lya/c.c)*1215.67d
    ...
    mask = (thismask EQ 1) AND $
           ...
           sciimg LT 5.0d5 AND $
           sciimg GT -1000.0d
    skymask1 = mask AND edgmask EQ 0 ;;AND lya_mask EQ 0 ; edge criteria
    ...
    ;;i1 = max(group)+1
      ...
      for iiter=1L, niter do begin
        ...
        FOR ii = 0L, objwork-1L DO BEGIN
            ...
            if max(wave) gt 0 then begin
               CASE objstruct[indx[iobj]].OBJID OF
                  id_fg: profmask = mask AND fitmask AND $
                                    NOT (waveimg GE lya_min_fg AND $
                                         waveimg LE lya_max_fg)
        ...
        IF n_fg GT 0 THEN BEGIN
           xtrace_fg = objstruct[indx[group[kk_fg]]].XPOS ## replicate(1.0, nx)
           lya_mask_fg = waveimg GE lya_min_fg AND $
                          waveimg LE lya_max_fg AND $
                          (xarr GE (xtrace_fg - pix_ap)) AND $
                          (xarr LE (xtrace_fg + pix_ap))
           FOR ll = 0L, objwork -1L DO lya_mask_fg = lya_mask_fg AND $
              NOT (obj_profiles[*, indx[group[ll]]] GT $
                   0.1*max(obj_profiles[*, indx[group[ll]]]))
        ENDIF ELSE lya_mask_fg = lonarr(nx, ny)
        ...
        skymask = skymask1 AND lya_mask_fg EQ 0 AND lya_mask_bg EQ 0
