...
IF N_ELEMENTS(uvalue) EQ 1 THEN CASE uvalue OF
 ...
 'go_trace': BEGIN
    ...
    tset_slits=mage_findslits('temp_trace.fits')
...
END 
 ...
 'go_pipe': BEGIN
    ...
    test = strsplit(stdfile[0],'\.gz',/extra,/regex)
    mage.stdfile=fileandpath(test[0])
    mage.sensfunc=stdoutfile[0]
    
    mage_pipe, mage, illumflatfile=illumflatfile, singles=singlesinter
 END
