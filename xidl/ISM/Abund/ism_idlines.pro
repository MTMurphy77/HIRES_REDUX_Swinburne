...
pro ism_idlines_icmmn, specfil, outfil, IFLG=iflg, clobber=clobber
...
pro ism_idlines_event, ev
  ...
  case uval of
     ...
     'LLIST': begin
      ...
      'VPLT' : begin
         x_velplt, state.specfil, state.zgal, inflg=state.inflg, $
                   /lls, /un_norm
      end
      'ZGAL' : begin
...
pro ism_idlines, specfil, outfil, zgal, LLIST=llist, $
                 XSIZE=xsize, YSIZE=ysize, INFLG=inflg, CLOBBER=clobber, $
                 UPDATE=update, INSTR=instr, FLG_LLST=flg_llst

  common ism_idlines_cmm
;
  if  N_params() LT 3  then begin 
    print,'Syntax - ' + $
      'ism_idlines, specfil, outfil, zgal, LLIST=, INFLG=, /CLOBBER, INSTR=, /UPDATE [v1.0]'
    return
  endif 
...
; Initialize the common blcok
  ...
  ism_idlines_icmmn, specfil, outfil, IFLG=inflg, CLOBBER=clobber
...
;  state.name_id = cw_field(civinf, title='Obj ', value=' ', xsize=18)
  ...
  a = where(abs(lines.wave-1215.6701) LT 1e-3, na)
  if na NE 1 then a = 0L ;; Just take the first line
  widget_control, state.droplist_id, set_droplist_select=a[0]
